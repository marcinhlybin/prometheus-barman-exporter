...
class BarmanCollector(object):
    ...
    def __init__(self, args):
        ...
        # Remove logging handlers to avoid opening barman.log over and over again
    ...
    def collect(self):
        ...
        for server_name in self.server_names:
            ...
            collectors['barman_backups_total'].add_metric([server_name], len(backups['status_done']) + len(backups['status_failed']))
            collectors['barman_backups_failed'].add_metric([server_name], len(backups['status_failed']))

            if  backups['status_done']:
                collectors['barman_last_backup'].add_metric([server_name], backups['status_done'][0]['end_time'])
                collectors['barman_first_backup'].add_metric([server_name], backups['status_done'][-1]['end_time'])

            for number, backup in enumerate(backups['status_done'], 1):
                collectors['barman_backups_size'].add_metric([server_name, str(number)], backup['size'])
                collectors['barman_backups_wal_size'].add_metric([server_name, str(number)], backup['wal_size'])
